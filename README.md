...
项目的核心思路，是将项目中的关键资产（包括各类文档、源代码、脚本、配置文件等）统一组织并构建为有向无环图（DAG），同时通过对资产状态（主要为 Clean / Dirty 两种状态）进行精细化管理，形成全局一致、可追溯、可审计的项目资产整体视图。

这一结构化、关系化的项目全景，将作为 LLM 上下文工程（Context Engineering） 的关键输入，帮助大模型更完整、准确地理解项目结构、依赖关系与业务语义，从而大幅提升 AI 在代码理解、生成、排查与合规校验等场景下的可靠性与准确性。
