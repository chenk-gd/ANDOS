...
export const TEST_AGENT_CONFIG: CreateAgentInput = {
  slug: 'test-agent',
  name: 'TestAgent',
...
export async function initializeTestAgent(): Promise<void> {
  const existing = await agentService.getAgentBySlug(TEST_AGENT_CONFIG.slug);

  if (!existing) {
    await agentService.createAgent(TEST_AGENT_CONFIG);
...
export async function generateTestSuite(
  ...
  const session = await agentService.createSession({
    agent_slug: TEST_AGENT_CONFIG.slug,
...
if (process.env.NODE_ENV === 'production') {
  initializeTestAgent().catch(console.error);
}
